...
interface ValueDateI{
year: number; month: number;
}
interface ValueUpdateStartTime{
  hours: number;
  minutes: number;
  seconds: number;
}
interface DatePickerData {
  ...
  // dateUtil:any,
  open?: boolean;
  showCustomRangeCalendars?: boolean;
  start?: Date|null;
  end?: Date|null;
  in_selection?: boolean;
  // startDate: any,
  // endDate: any
  // changeRightMonth({}:any): any,
...
export default mixins(dateUtilMixin).extend({
  ...
  props: {
    ...
    calculatePosition: {
       ...
       */
      default(dropdownList: any, component: any, { width, top, left, right }: ContainerPropsI) {
  ...
  data(): DatePickerData {
    // return {
    //   locale: getDateUtil().localeData({ ...this.localeData }),
    //   start:new Date()
    // }
  ...
  const data: DatePickerData = { 
    ...
    const startDate = this.dateRange.startDate || null;
    const endDate = this.dateRange.endDate || null;
  ...
  methods: {
    dateFormatFn(classes: any, date: Date): any {
    ...
    changeLeftMonth(value: ValueDateI) {
    ...
    changeRightMonth(value: ValueDateI) {
      const newDate = new Date(value.year, value.month - 1, 1);
    ...
    normalizeDatetime(value: string|Date, oldValue: Date) {
    ...
    hoverDate(value: string) {
      if (this.readonly || !this.end || !this.start) return false;
    ...
    togglePicker(value: boolean|null, event: any) {
    ...
    clickAway($event: any) {
      if (
        ...
        this.$refs.dropdown &&
        !(this.$refs.dropdown as any).contains($event.target)
      ) {
        this.clickCancel();
      }
    },
    clickRange(value: any) {
      this.in_selection = false;
    ...
    onUpdateStartTime(value: ValueUpdateStartTime) {
    ...
    onUpdateEndTime(value: ValueUpdateStartTime) {
      if(!this.end) return;
  ...
  computed: {
    ...
    showCalendars() {
      const vm: any = this;
      return this.alwaysShowCalendars || vm.showCustomRangeCalendars;
    },
    startText() {
      if (this.start === null) return "";
      const vm: any = this;
      return this.$dateUtil.format(vm.start, vm.locale.format);
    },
    endText() {
      if (this.end === null) return "";
      const vm: any = this;
      return this.$dateUtil.format(vm.end, vm.locale.format);
    },
    rangeText() {
      const vm: any = this;
...
const vm: any = this;
      return (
        !vm.isClear &&
        (vm.start.getTime() !== origStart.getTime() ||
          vm.end.getTime() !== origEnd.getTime())
      );
    },
  },
  watch: {
